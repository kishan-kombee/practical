...
final class Table extends PowerGridComponent
{
    ...
    public string $sortField = 'categories.id';
    ...
    public function __construct()
    {
        ...
        $this->tableName = __('messages.category.listing.tableName');
    ...
    public function exportData()
    {
        try {
            ...
            $exportService = ExportServiceFactory::create('category');
            ...
            $exportParams = [
                'exportType' => 'category', // Export type identifier
            ...
            logger()->error('App\Livewire\CategoryTable: exportData: Throwable', ['Message' => $e->getMessage(), 'TraceAsString' => $e->getTraceAsString()]);
            session()->flash('error', __('messages.category.messages.common_error_message'));
    ...
    public function header(): array
    {
        ...
        if (Gate::allows('add-category')) {
            $headerArray[] = Button::add('add-category')
                ->slot('    <a href="/category/create" title="Add New Category" data-testid="add_new" class="flex items-center justify-center cursor-pointer" wire:navigate>
        ...
        if (Gate::allows('export-category')) {
            $headerArray[] = Button::add('export-data')
                ->slot('<a wire:click.prevent="exportData" href="#" title="Export Category" data-testid="export_button" data-export-type="category"  class="flex items-center justify-center"  wire:target="exportData" onclick="handleExportClick(event)" x-bind:class="$wire.isExporting ? \'opacity-50 cursor-not-allowed\' : \'cursor-pointer\'" x-bind:disabled="$wire.isExporting" >
        ...
        if (Gate::allows('bulkDelete-category')) {
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(__('messages.category.listing.id'), 'id')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.category.listing.name'), 'name')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.category.listing.status'), 'status')
                ->sortable()
                ->searchable(),
            Column::make(__('messages.created_date'), 'created_at_formatted', 'created_at'),
            Column::action(__('messages.category.listing.actions')),
        ];
    }
    ...
    public function filters(): array
    {
        return [
            ...
            Filter::inputText('name', 'categories.name')
                ->operators(['contains'])
                ->placeholder(__('messages.category.filter.name_placeholder')),
            Filter::select('status', 'status')
                ->dataSource(Category::status())
                ->optionLabel('label')
                ->optionValue('key'),
    ...
    {
        return $this->redirect('category/' . $id . '/edit', navigate: true); // redirect to edit component
    }
    ...
    public function actions(Category $row): array
    {
        ...
        if (Gate::allows('show-category')) {
            $actions[] = Button::add('view')
                ...
                ->dispatchTo('category.show', 'show-category-info', ['id' => $row->id]);
        }
        ...
        if (Gate::allows('delete-category')) {
            $actions[] = Button::add('delete-category')
                ...
                ->dispatchTo('category.delete', 'delete-confirmation', ['ids' => [$row->id], 'tableName' => $this->tableName]);
        }
