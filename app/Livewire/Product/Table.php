...
final class Table extends PowerGridComponent
{
    ...
    public function exportData()
    {
        try {
            ...
            $exportService = ExportServiceFactory::create('product');
            ...
            $exportParams = [
                'exportType' => 'product', // Export type identifier
    ...
    public function header(): array
    {
        ...
        if (Gate::allows('export-product')) {
            $headerArray[] = Button::add('export-data')
                ->slot('<a wire:click.prevent="exportData" href="#" title="Export Product" data-testid="export_button" data-export-type="product"  class="flex items-center justify-center"  wire:target="exportData" onclick="handleExportClick(event)" x-bind:class="$wire.isExporting ? \'opacity-50 cursor-not-allowed\' : \'cursor-pointer\'" x-bind:disabled="$wire.isExporting" >
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ->add('id')
            ->add('description_formatted', function ($row) {
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(__('messages.product.listing.item_code'), 'item_code')
                ->sortable()
                ->searchable(),
            ...
            Column::make(__('messages.product.listing.price'), 'price')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.product.listing.description'), 'description_formatted', 'description')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.product.listing.categories'), 'categories_name')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.product.listing.sub_categories'), 'sub_categories_name')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.product.listing.available_status'), 'available_status')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.product.listing.quantity'), 'quantity')
                ->sortable()
                ->searchable(),
    ...
    public function filters(): array
    {
        return [
            ...
            Filter::inputText('item_code', 'products.item_code')
                ->operators(['contains'])
                ->placeholder(__('messages.product.filter.item_code_placeholder')),
            Filter::inputText('name', 'products.name')
                ->operators(['contains'])
                ->placeholder(__('messages.product.filter.name_placeholder')),
            Filter::inputText('price', 'products.price')
                ->operators(['contains'])
                ->placeholder(__('messages.product.filter.price_placeholder')),
            Filter::select('available_status', 'products.available_status')
                ->dataSource(Product::available_status())
                ->optionLabel('label')
                ->optionValue('key'),
            Filter::inputText('quantity', 'products.quantity')
                ->operators(['contains'])
                ->placeholder(__('messages.product.filter.quantity_placeholder')),
