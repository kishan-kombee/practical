...
final class Table extends PowerGridComponent
{
    ...
    public string $sortField = 'sub_categories.id';
    ...
    public function __construct()
    {
        ...
        $this->tableName = __('messages.sub_category.listing.tableName');
    ...
    public function exportData()
    {
        try {
            ...
            $exportService = ExportServiceFactory::create('subcategory');
            ...
            $exportParams = [
                'exportType' => 'subcategory', // Export type identifier
            ...
            logger()->error('App\Livewire\SubCategoryTable: exportData: Throwable', ['Message' => $e->getMessage(), 'TraceAsString' => $e->getTraceAsString()]);
            session()->flash('error', __('messages.subcategory.messages.common_error_message'));
    ...
    public function header(): array
    {
        ...
        if (Gate::allows('add-sub_category')) {
            $headerArray[] = Button::add('add-sub_category')
                ->slot('    <a href="/sub_category/create" title="Add New SubCategory" data-testid="add_new" class="flex items-center justify-center cursor-pointer" wire:navigate>
        ...
        if (Gate::allows('export-sub_category')) {
            $headerArray[] = Button::add('export-data')
                ->slot('<a wire:click.prevent="exportData" href="#" title="Export SubCategory" data-testid="export_button" data-export-type="subcategory"  class="flex items-center justify-center"  wire:target="exportData" onclick="handleExportClick(event)" x-bind:class="$wire.isExporting ? \'opacity-50 cursor-not-allowed\' : \'cursor-pointer\'" x-bind:disabled="$wire.isExporting" >
        ...
        if (Gate::allows('bulkDelete-sub_category')) {
            $headerArray[] = Button::add('bulk-delete')
                ...
                <div class="flex rounded-md ring-1 transition focus:ring-2
                    ...
                    title="Bulk Delete SubCategorys">
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(__('messages.sub_category.listing.id'), 'id')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.sub_category.listing.categories'), 'categories_name')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.sub_category.listing.name'), 'name')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.sub_category.listing.status'), 'status')
                ->sortable()
                ->searchable(),
            Column::make(__('messages.created_date'), 'created_at_formatted', 'sub_categories.created_at'),
            Column::action(__('messages.sub_category.listing.actions')),
        ];
    }
    ...
    public function filters(): array
    {
        return [
            ...
            Filter::inputText('name', 'sub_categories.name')
                ->operators(['contains'])
                ->placeholder(__('messages.sub_category.filter.name_placeholder')),
            Filter::select('status', 'sub_categories.status')
                ->dataSource(SubCategory::status())
                ->optionLabel('label')
                ->optionValue('key'),
            Filter::datetimepicker('created_at', 'sub_categories.created_at'),
        ];
    }
    ...
    {
        return $this->redirect('sub_category/' . $id . '/edit', navigate: true); // redirect to edit component
    }
    ...
    public function actions(SubCategory $row): array
    {
        ...
        if (Gate::allows('show-sub_category')) {
            $actions[] = Button::add('view')
                ...
                ->dispatchTo('sub-category.show', 'show-subcategory-info', ['id' => $row->id]);
        }
        ...
        if (Gate::allows('delete-sub_category')) {
            $actions[] = Button::add('delete-sub-category')
                ...
                ->dispatchTo('sub-category.delete', 'delete-confirmation', ['ids' => [$row->id], 'tableName' => $this->tableName]);
        }
