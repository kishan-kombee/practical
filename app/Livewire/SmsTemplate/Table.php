...
final class Table extends PowerGridComponent
{
    ...
    public function exportData()
    {
        try {
            ...
            $exportService = ExportServiceFactory::create('smstemplate');
            ...
            $exportParams = [
                'exportType' => 'smstemplate', // Export type identifier
    ...
    public function header(): array
    {
        ...
        if (Gate::allows('export-sms-template')) {
            $headerArray[] = Button::add('export-data')
                ->slot('<a wire:click.prevent="exportData" href="#" title="Export SmsTemplate" data-testid="export_button" data-export-type="smstemplate"  class="flex items-center justify-center"  wire:target="exportData" onclick="handleExportClick(event)" x-bind:class="$wire.isExporting ? \'opacity-50 cursor-not-allowed\' : \'cursor-pointer\'" x-bind:disabled="$wire.isExporting" >
    ...
    public function filters(): array
    {
        return [
            ...
            Filter::inputText('label', 'sms_templates.label')
                ->operators(['contains'])
                ->placeholder(__('messages.sms_template.filter.label_placeholder')),

            Filter::inputText('dlt_message_id', 'sms_templates.dlt_message_id')
                ->operators(['contains'])
                ->placeholder(__('messages.sms_template.filter.dlt_message_id_placeholder')),
